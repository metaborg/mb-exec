...
public class Generic_Def extends AbstractNode implements I_Def, IGenericNode
{ 
  public IStrategoTerm aterm;

  public Generic_Def (INodeSource source, IStrategoTerm term) 
  { 
  ...
  { 
    ...
    { 
      ...
      { 
        I_Node replacement = replace(new AnnoDef_2(source, (INodeList)NodeUtils.makeList(term.getSubterm(0).getSubtermCount(), term.getSubterm(0), Generic_Anno.class), new Generic_StrategyDef(source, term.getSubterm(1))));
        if(depth > 0)
        { 
    ...
    { 
      ...
      replacement = new Generic_StrategyDef(getSourceInfo(), aterm);
      return (I_Node)replace(replacement).specialize(1);
    }
    catch(RewritingException y_177429)
    { }
