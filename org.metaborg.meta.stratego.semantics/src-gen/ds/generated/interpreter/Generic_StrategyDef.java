...
public class Generic_StrategyDef extends AbstractNode implements I_StrategyDef, IGenericNode
{ 
  public IStrategoTerm aterm;

  public Generic_StrategyDef (INodeSource source, IStrategoTerm term) 
  { 
  ...
  { 
    ...
    { 
      ...
      { 
        I_Node replacement = replace(new SDefT_4(source, Tools.asJavaString(term.getSubterm(0)), (INodeList)NodeUtils.makeList(term.getSubterm(1).getSubtermCount(), term.getSubterm(1), Generic_Typedid.class), (INodeList)NodeUtils.makeList(term.getSubterm(2).getSubtermCount(), term.getSubterm(2), Generic_Typedid.class), new Generic_Strategy(source, term.getSubterm(3))));
        if(depth > 0)
        { 
      ...
      { 
        ...
        { 
          I_Node replacement = replace(new ExtSDef_3(source, Tools.asJavaString(term.getSubterm(0)), (INodeList)NodeUtils.makeList(term.getSubterm(1).getSubtermCount(), term.getSubterm(1), Generic_Typedid.class), (INodeList)NodeUtils.makeList(term.getSubterm(2).getSubtermCount(), term.getSubterm(2), Generic_Typedid.class)));
          if(depth > 0)
          { 
            replacement.specializeChildren(depth - 1);
          }
