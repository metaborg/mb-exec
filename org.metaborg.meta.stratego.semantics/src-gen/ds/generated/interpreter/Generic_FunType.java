...
public class Generic_FunType extends AbstractNode implements I_FunType, IGenericNode
{ 
  public IStrategoTerm aterm;

  public Generic_FunType (INodeSource source, IStrategoTerm term) 
  { 
  ...
  { 
    ...
    { 
      ...
      if(name.equals("FunType") && term.getSubtermCount() == 2)
      { 
        I_Node replacement = replace(new FunType_2(source, (INodeList)NodeUtils.makeList(term.getSubterm(0).getSubtermCount(), term.getSubterm(0), Generic_Type.class), new Generic_Type(source, term.getSubterm(1))));
        if(depth > 0)
        { 
