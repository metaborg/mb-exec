...
public class Generic_Sort extends AbstractNode implements I_Sort, IGenericNode
{ 
  public IStrategoTerm aterm;

  public Generic_Sort (INodeSource source, IStrategoTerm term) 
  { 
  ...
  { 
    ...
    { 
      ...
      { 
        I_Node replacement = replace(new Sort_2(source, Tools.asJavaString(term.getSubterm(0)), (INodeList)NodeUtils.makeList(term.getSubterm(1).getSubtermCount(), term.getSubterm(1), Generic_Sort.class)));
        if(depth > 0)
        { 
      ...
      { 
        ...
        { 
          I_Node replacement = replace(new SortVar_1(source, Tools.asJavaString(term.getSubterm(0))));
          if(depth > 0)
          { 
            replacement.specializeChildren(depth - 1);
          }
        ...
        { 
          ...
          { 
            I_Node replacement = replace(new SortNoArgs_1(source, Tools.asJavaString(term.getSubterm(0))));
            if(depth > 0)
            { 
              replacement.specializeChildren(depth - 1);
            }
