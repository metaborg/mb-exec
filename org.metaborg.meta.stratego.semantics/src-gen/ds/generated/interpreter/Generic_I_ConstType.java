...
public class Generic_I_ConstType extends AbstractNode implements I_ConstType,
		IGenericNode {
	public IStrategoTerm aterm;

	public Generic_I_ConstType(INodeSource source, IStrategoTerm term) {
	...
	@Override
	public I_ConstType specialize(int depth) {
		if (aterm instanceof IStrategoAppl) {
			...
			if (name.equals("ConstType") && term.getSubtermCount() == 1) {
				I_ConstType replacement = replace(new ConstType_1(source,
						new Generic_I_Sort(source, term.getSubterm(0))));
				if (depth > 0) {
