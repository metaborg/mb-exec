...
public class Generic_Type extends AbstractNode implements I_Type, IGenericNode
{ 
  public IStrategoTerm aterm;

  public Generic_Type (INodeSource source, IStrategoTerm term) 
  { 
  ...
  { 
    ...
    { 
      ...
      replacement = new Generic_ConstType(getSourceInfo(), aterm);
      return (I_Node)replace(replacement).specialize(1);
    }
    catch(RewritingException z_177429)
    { 
      try
      { 
        if(replacement != null)
        ...
        replacement = new Generic_FunType(getSourceInfo(), aterm);
        return (I_Node)replace(replacement).specialize(1);
      }
      catch(RewritingException a_177430)
      { }
    }
