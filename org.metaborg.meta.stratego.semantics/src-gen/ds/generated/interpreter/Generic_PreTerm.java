...
public class Generic_PreTerm extends AbstractNode implements I_PreTerm, IGenericNode
{ 
  public IStrategoTerm aterm;

  public Generic_PreTerm (INodeSource source, IStrategoTerm term) 
  { 
  ...
  { 
    ...
    { 
      ...
      { 
        I_Node replacement = replace(new As_2(source, new Generic_Var(source, term.getSubterm(0)), new Generic_PreTerm(source, term.getSubterm(1))));
        if(depth > 0)
        { 
      ...
      { 
        ...
        { 
          I_Node replacement = replace(new Explode_2(source, new Generic_STerm(source, term.getSubterm(0)), new Generic_STerm(source, term.getSubterm(1))));
          if(depth > 0)
          { 
            replacement.specializeChildren(depth - 1);
          }
        ...
        { 
          ...
          { 
            I_Node replacement = replace(new Op_2(source, Tools.asJavaString(term.getSubterm(0)), (INodeList)NodeUtils.makeList(term.getSubterm(1).getSubtermCount(), term.getSubterm(1), Generic_STerm.class)));
            if(depth > 0)
            { 
              replacement.specializeChildren(depth - 1);
            }
          ...
          { 
            ...
            { 
              I_Node replacement = replace(new Str_1(source, Tools.asJavaString(term.getSubterm(0))));
              if(depth > 0)
              { 
                replacement.specializeChildren(depth - 1);
              }
            ...
            { 
              ...
              { 
                I_Node replacement = replace(new Int_1(source, Tools.asJavaString(term.getSubterm(0))));
                if(depth > 0)
                { 
                  replacement.specializeChildren(depth - 1);
                }
              ...
              { 
                ...
                { 
                  I_Node replacement = replace(new Real_1(source, Tools.asJavaString(term.getSubterm(0))));
                  if(depth > 0)
                  { 
                    replacement.specializeChildren(depth - 1);
                  }
                ...
                { 
                  ...
                  { 
                    I_Node replacement = replace(new Anno_2(source, new Generic_PreTerm(source, term.getSubterm(0)), new Generic_PreTerm(source, term.getSubterm(1))));
                    if(depth > 0)
                    { 
                      replacement.specializeChildren(depth - 1);
                    }
    ...
    { 
      ...
      replacement = new Generic_Var(getSourceInfo(), aterm);
      return (I_Node)replace(replacement).specialize(1);
    }
    catch(RewritingException w_177429)
    { 
      try
      { 
        if(replacement != null)
        ...
        replacement = new Generic_Wld(getSourceInfo(), aterm);
        return (I_Node)replace(replacement).specialize(1);
      }
      catch(RewritingException x_177429)
      { }
    }
