...
public class Main {
	...
	public static void main(String args[]) throws IOException {
		...
		try {
			...
			// System.out.println("Load time: " + (System.nanoTime() -
			// loadTime)/1000/1000 + "ms");
		} catch (IOException e) {
			e.printStackTrace();
	...
	private static Interpreter createInterpreter(String factoryName) {
		...
		try {
			Class<ITermFactory> clazz = (Class<ITermFactory>) Class
					.forName(factoryName);
			Constructor<ITermFactory> ctor = clazz.getConstructor();
			ITermFactory factory = ctor.newInstance();
			return new Interpreter(factory, factory); // new
														// BasicTermFactory());
		...
		System.err.println("Fatal: failed to instantiate term factory '"
				+ factoryName + "'");
		System.exit(-2);
		return null;
	}

	private static void loadLibraries(Interpreter itp,
			List<String> libraryArgs, ITermFactory factory) {
	...
	private static IOperatorRegistry findOperatorRegistry(String s,
		...
		try {
			...
			final Constructor<IOperatorRegistry> m = (Constructor<IOperatorRegistry>) clazz
